...
// var gSocketAddr = 
// "wss://ldai1efi.wdf.sap.corp:44318/sap/bc/apc/sap/z_eezz_ws_push_channel";
...
function eezzConnect() {
    ...
    eezzWebSocket.onopen = function() { 
        ...
        var aJson     = {"path": aParser.pathname, "args": eezzArguments, "document": markup};                
    ...
    eezzWebSocket.onmessage = function(aEvent) {
        ...
        for (xKeyElement in aJson.update) {
            ...
            if (xDstElement[0].getAttribute('class') != null) {
                if (xDstElement[0].getAttribute('class').indexOf('eezzTreeNode') >= 0) {
                    if (xDestination.length > 2) {
...
function easyClick(aEvent, aElement) {
    ...
    if (aJson.callback) {    
        ...
        if (aElement.tagName == 'TR') {
            ...
            for (var i = 0; i < xElementList.length; i++) {
                if (xElementList[i].parentElement === aElement.parentElement) {
                    // ( /(?:^|\s)eezzSelected(?!\S)/g , '' )
    ...
    var xElementClass = aElement.getAttribute('class');
    if (xElementClass != null) {
        if (xElementClass.indexOf('eezzTree') >= 0) {
            if (xElementClass.indexOf('eezzTreeNode') >= 0) {
